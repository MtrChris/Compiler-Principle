...
inline string TerminalElement::getType() const
{
    return type;
}

inline string TerminalElement::getVal() const
{
    return val;
}

inline string NonTerminalElement::getName() const
{
    return name;
}
...
ElementDict elemDict;
void GrammarParser::LR1Main(const vector<AlgElement *>& input)
{
    ...
    while (true) {
        ...
        AlgElement *currentSymbol = input[currentPosition];
        int currentSymbolNum = elemDict.findElem(*currentSymbol);
        ...
        switch (item.action) {
            case SHIFT:
                ...
                if (currentSymbol->getIsTerminal())
                {
                    TerminalElement* shiftAlg = dynamic_cast<TerminalElement*>(currentSymbol);
                    shiftE->place = shiftAlg->getVal();
                }
                else
                {
                    NonTerminalElement* shiftAlg = dynamic_cast<NonTerminalElement*>(currentSymbol);
                    shiftE->place = shiftAlg->getName();
                }
            ...
            case REDUCE: {
                int productionIndex = item.index;
                ProductionAlg prod = getProductionAlg(productionIndex);

                int gotoState = chart.get(stateStack.top(), currentSymbolNum).index;
                stateStack.push(gotoState);
                codegenerator.GenerateCode(nextStat, prod, stateStack, symbolStack);
                break;
            }
